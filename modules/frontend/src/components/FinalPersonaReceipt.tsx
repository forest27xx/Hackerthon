...
export type FinalPersonaReceiptProps = {
  entries: CartEntry[];
...
const receiptGroups: Record<ReceiptGroup, { theme: ReceiptTheme; title: string }> = {
  HC: { theme: "magic", title: "快乐下单事务所" },
  HE: { theme: "anime", title: "快乐下单事务所" },
  NC: { theme: "plush", title: "快乐下单事务所" },
  NE: { theme: "ink", title: "快乐下单事务所" }
};

const receiptGroupOrder: ReceiptGroup[] = ["HC", "HE", "NC", "NE"];
const ignoredOptionLabels = new Set([
  "按商品默认",
  "默认口味",
  "默认奶基",
  "默认汤底",
...
const getOptionSummary = (entry: CartEntry) =>
  getSelectedOptionChoices(entry)
    ...
    .filter((label) => !ignoredOptionLabels.has(label))
    .join(" / ");
...
export function FinalPersonaReceipt({
  ...
  const itemRowsHeight = receiptItems.reduce((sum, item) => {
  ...
  return (
    <section className={`final-persona-receipt ${group.theme}`} aria-label="快乐下单事务所小票">
      ...
      >
        ...
        <section className="final-money-ledger">
          ...
          <div>
            ...
            <b>{baseCouponDiscount > 0 ? `-${money(baseCouponDiscount)}` : "0.00"}</b>
          </div>
          {couponBoostDiscount > 0 && (
            <div>
              <span>神券膨胀</span>
              <b>-{money(couponBoostDiscount)}</b>
            </div>
          )}
          <div className="final-payable">
            <span>实付总额</span>
            <strong>¥{money(computedPayablePrice)}</strong>
          </div>
        </section>
      ...
      <div className="receipt-style-switcher" aria-label="切换小票样式">
        <button type="button" onClick={() => switchGroup(-1)} aria-label="上一种小票样式">
          {"<"}
        </button>
        <div>
        ...
        <button type="button" onClick={() => switchGroup(1)} aria-label="下一种小票样式">
          {">"}
        </button>
