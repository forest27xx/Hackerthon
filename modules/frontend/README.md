...
手机预览二维码见：[docs/assets/eati-mobile-qr.png](docs/assets/eati-mobile-qr.png)。
...
4. 点击 `下单`  
   下单后进入“订单进行中”沉浸式场景页。用户仍然是下单人，只是在商家备餐、骑手沟通、收餐确认中做选择。

5. 完成 8 个订单节点
   每局从 100 条自适应题库中抽取 8 个节点：商家 4 个、骑手 3 个、收餐 1 个。题目会根据本单商品过滤，没点奶茶就不会出现珍珠/封口题。
...
src/data/catalog.ts         # 商品、分类、combo 内容
src/data/foodPersonas.ts    # 16 型吃商人格文案和头像映射
src/lib/orderProgressEngine.ts # 订单进行中题库、筛选和进度逻辑
src/lib/gameEngine.ts       # 分数、combo、美食人格、结果摘要算法
